...
const defaultCommands_1 = require("./defaultCommands");
const helpRenderer_1 = require("./helpRenderer");
/**
 * 슬래시 커맨드 매니저 클래스
 */
class SlashCommandManager {
    ...
    constructor(context, llmService, services) {
        ...
        (0, helpRenderer_1.setExtensionContext)();
        // 기본 명령어 등록
    ...
    registerDefaultCommands() {
        (0, defaultCommands_1.createDefaultCommands)(this.services).forEach(command => {
    ...
    async executeCommand(input) {
        ...
        if (!command) {
            ...
            if (similarCommands.length > 0 && similarCommands[0].distance <= 1) {
                ...
                const result = await vscode.window.showInformationMessage(`명령어 '/${commandName}'를 찾을 수 없습니다. '/${closestCommand.name}'를 실행하시겠습니까?`, '실행', '취소');
                if (result === '실행') {
...
`;
    ...
    async showLlmResponsePanel(title, content) {
        // WebView 패널 생성
        const panel = vscode.window.createWebviewPanel('apeLlmResponse', title, vscode.ViewColumn.One, {
            enableScripts: true,
     ...
     * 하위 명령어 제안 항목 가져오기
     */
    getSubCommandSuggestions(mainCommand, subCommand) {
        const suggestions = [];
        // 메인 명령어 찾기
        ...
        if (subCommand) {
            subCommands = subCommands.filter(sc => sc.name.toLowerCase().includes(subCommand.toLowerCase()));
        }
        // 하위 명령어 제안 항목 생성
    ...
    async showCommandHelp(arg) {
        // 도움말 패널 생성
        const helpPanel = vscode.window.createWebviewPanel('apeCommandHelp', 'APE 도움말', vscode.ViewColumn.One, {
        ...
        if (!arg) {
            ...
            helpPanel.webview.html = await (0, helpRenderer_1.generateHelpHtml)();
        }
        else if (arg.startsWith('guide ')) {
            // 가이드 문서 표시
            const guideId = arg.substring(6);
            helpPanel.webview.html = await (0, helpRenderer_1.generateGuideHtml)(guideId);
        }
        else if (arg === 'guides' || arg === 'guide') {
            // 가이드 목록 표시
            helpPanel.webview.html = await (0, helpRenderer_1.generateGuidesListHtml)();
        }
        else if (arg === 'faq') {
            // FAQ 표시
            helpPanel.webview.html = await (0, helpRenderer_1.generateFaqHtml)();
        }
        else if (arg === 'tools') {
            // Agent 도구 목록 표시
            helpPanel.webview.html = await (0, helpRenderer_1.generateToolsHelpHtml)();
        }
        else if (this.getCommand(arg)) {
            // 명령어 상세 정보 표시
            helpPanel.webview.html = await (0, helpRenderer_1.generateCommandDetailHtml)(arg);
        }
            ...
            if (this.llmService) {
                ...
                helpPanel.webview.html = await (0, helpRenderer_1.generateSmartHelpHtml)(query, this.llmService);
            }
            else {
            ...
            helpPanel.webview.html = await (0, helpRenderer_1.generateHelpHtml)(arg);
        }
    }
    /**
     * 도움말 명령어 처리
    ...
    async showGitStatus() {
        try {
            ...
            // 도움말 패널로 표시
            const statusPanel = vscode.window.createWebviewPanel('apeGitStatus', 'Git 상태', vscode.ViewColumn.One, {
                enableScripts: false,
                localResourceRoots: []
            });
