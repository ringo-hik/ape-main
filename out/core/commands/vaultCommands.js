...
exports.createVaultCommands = createVaultCommands;
...
async function createContextOrItem(vaultService, typeStr, name) {
    try {
        ...
        switch (typeStr.toLowerCase()) {
            case 'system':
            case '시스템':
                contextType = vaultService_1.VaultContextType.System;
                break;
            case 'project':
            case '프로젝트':
                contextType = vaultService_1.VaultContextType.Project;
                break;
            case 'personal':
            case '개인':
                contextType = vaultService_1.VaultContextType.Personal;
                break;
            case 'shared':
            case '공유':
                contextType = vaultService_1.VaultContextType.Shared;
                break;
            case 'template':
            case '템플릿':
                contextType = vaultService_1.VaultContextType.Template;
                break;
            default:
...
async function deleteContextOrItem(vaultService, id) {
    try {
        ...
        if (context) {
            // 컨텍스트 삭제 확인
            const confirmed = await vscode.window.showWarningMessage(`"${context.name}" 컨텍스트를 삭제하시겠습니까? 이 작업은 되돌릴 수 없으며, 포함된 모든 아이템이 함께 삭제됩니다.`, { modal: true }, '삭제');
            if (confirmed !== '삭제') {
                await vscode.commands.executeCommand('ape.sendLlmResponse', {
                    role: 'assistant',
        ...
        for (const ctx of allContexts) {
            ...
            if (itemIndex !== -1) {
                ...
                // 아이템 삭제 확인
                const confirmed = await vscode.window.showWarningMessage(`"${item.name}" 아이템을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`, { modal: true }, '삭제');
                if (confirmed !== '삭제') {
                    await vscode.commands.executeCommand('ape.sendLlmResponse', {
                        role: 'assistant',
...
function getContextTypeKorean(type) {
    switch (type) {
        case vaultService_1.VaultContextType.System: return '시스템';
        case vaultService_1.VaultContextType.Project: return '프로젝트';
        case vaultService_1.VaultContextType.Personal: return '개인';
        case vaultService_1.VaultContextType.Shared: return '공유';
        case vaultService_1.VaultContextType.Template: return '템플릿';
...
//# sourceMappingURL=vaultCommands.js.map