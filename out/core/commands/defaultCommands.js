...
function createDefaultCommands(services) {
    ...
    if (services?.vaultService) {
        const vaultCommands = (0, vaultCommands_1.createVaultCommands)(services.vaultService);
...
예시: \`/chat save\`, \`/chat list\`, \`/chat show chat_12345\``
            ...
            if (subCommand === 'save' || subCommand === '저장') {
                ...
                try {
                    ...
                    // 파일 저장 (채팅 내역)
                    await vscode.workspace.fs.writeFile(chatHistoryPath, Buffer.from(markdown, 'utf8'));
                    // 파일 저장 (메타데이터)
                    await vscode.workspace.fs.writeFile(metadataPath, Buffer.from(JSON.stringify(metadata, null, 2), 'utf8'));
                    // 트리 뷰 새로고침
                    try {
