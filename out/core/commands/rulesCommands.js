...
exports.createRulesCommands = createRulesCommands;
...
async function deleteRule(rulesService, ruleName) {
    try {
        ...
        // 삭제 확인 (프롬프트 대화상자 사용)
        const confirmation = await vscode.window.showWarningMessage(`'${rule.name}' Rule을 삭제하시겠습니까?`, { modal: true }, '삭제', '취소');
        if (confirmation !== '삭제') {
            await vscode.commands.executeCommand('ape.sendLlmResponse', {
                role: 'assistant',
...
//# sourceMappingURL=rulesCommands.js.map