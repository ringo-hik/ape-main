...
class TodoService {
    ...
    todoItems = [];
    todoStoragePath;
    _onDidChangeTodoList = new vscode.EventEmitter();
    onDidChangeTodoList = this._onDidChangeTodoList.event;
    constructor(context) {
    ...
    loadTodoItems() {
        try {
            if (fs.existsSync(this.todoStoragePath)) {
                ...
                // 날짜 형식 복원
                this.todoItems = todoData.map((item) => ({
                    ...item,
    ...
    getTodoList(filter, sort) {
        ...
        if (sort) {
            ...
            filteredItems = this.sortTodoItems(filteredItems, {
                by: todo_1.TodoSortBy.PRIORITY,
                direction: todo_1.SortDirection.DESC
            });
        }
        return {
            items: filteredItems,
            pendingCount: this.todoItems.filter(item => item.status === todo_1.TodoStatus.PENDING || item.status === todo_1.TodoStatus.IN_PROGRESS).length,
            completedCount: this.todoItems.filter(item => item.status === todo_1.TodoStatus.COMPLETED).length
        };
    }
    /**
     * 할 일 항목 필터링
     ...
     * 할 일 항목 정렬
     */
    sortTodoItems(items, options) {
        const { by, direction } = options;
        const directionMultiplier = direction === todo_1.SortDirection.ASC ? 1 : -1;
        return [...items].sort((a, b) => {
            let comparison = 0;
            switch (by) {
                case todo_1.TodoSortBy.PRIORITY: {
                    // 우선순위 매핑: HIGH(3), MEDIUM(2), LOW(1)
                    const priorityValues = {
                        [todo_1.TodoPriority.HIGH]: 3,
                        [todo_1.TodoPriority.MEDIUM]: 2,
                        [todo_1.TodoPriority.LOW]: 1
                    };
                    comparison = (priorityValues[a.priority] - priorityValues[b.priority]) * directionMultiplier;
                    break;
                }
                case todo_1.TodoSortBy.DUE_DATE:
    ...
    updateTodoItem(id, updates) {
        ...
        if (updates.status === todo_1.TodoStatus.COMPLETED && this.todoItems[index].status !== todo_1.TodoStatus.COMPLETED) {
