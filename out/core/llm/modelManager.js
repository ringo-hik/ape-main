...
class ModelManager {
    _context;
    // 현재 활성 모델
    _activeModel = models_1.ModelId.GPT_4_1_MINI;
    ...
    _loadConfiguration() {
        try {
            ...
            const configModel = config.get('defaultModel', models_1.ModelId.GPT_4_1_MINI);
            // 유효한 모델인지 확인
            if (this._isValidModel(configModel)) {
                ...
                console.warn(`유효하지 않은 모델: ${configModel}, 기본 모델 사용: ${models_1.ModelId.GPT_4_1_MINI}`);
                // 활성 모델이 이미 기본 모델이 아닌 경우에만 업데이트
                if (this._activeModel !== models_1.ModelId.GPT_4_1_MINI) {
                    const oldModel = this._activeModel;
                    this._activeModel = models_1.ModelId.GPT_4_1_MINI;
                    // 설정 업데이트 플래그 설정
                    this._isUpdatingConfig = true;
                    // 설정 업데이트
                    Promise.resolve(config.update('defaultModel', models_1.ModelId.GPT_4_1_MINI, vscode.ConfigurationTarget.Global))
                        .then(() => {
                        // 모델 변경 이벤트 발생
                        this._onDidChangeModel.fire({
                            oldModel,
                            newModel: models_1.ModelId.GPT_4_1_MINI
                        });
                        console.log(`기본 모델로 설정 업데이트됨: ${models_1.ModelId.GPT_4_1_MINI}`);
                    })
                        .then(undefined, (err) => {
    ...
    _isValidModel(modelId) {
        // 표준 모델 확인
        const isStandardModel = Object.values(models_1.ModelId).includes(modelId);
    ...
    getAvailableModels() {
        try {
            ...
            // 표준 모델 반환 (내부망 모델 제외)
            return Object.values(models_1.ModelId);
        }
        catch (error) {
    ...
    getModelDisplayName(modelId) {
        ...
        if (Object.values(models_1.ModelId).includes(modelId)) {
            return models_1.ModelDisplayNames[modelId];
        }
    ...
    getModelDescription(model) {
        ...
        if (Object.values(models_1.ModelId).includes(model)) {
            return models_1.ModelDescriptions[model];
        }
        // 기존 하드코딩된 매핑 방식 사용 (이전 버전과의 호환성 유지)
        switch (model) {
            case 'openai/gpt-4.1-mini':
