...
exports.AutoCommitService = void 0;
...
class AutoCommitService {
    ...
    registerCommands() {
        // 자동 커밋 토글
        this.disposables.push(vscode.commands.registerCommand('ape.git.toggleAutoCommit', () => this.toggleAutoCommit()));
        // 수동 커밋
        this.disposables.push(vscode.commands.registerCommand('ape.git.commit', () => this.createCommit()));
        // 임시 커밋 통합
        this.disposables.push(vscode.commands.registerCommand('ape.git.consolidateTemporaryCommits', () => this.consolidateTemporaryCommits()));
    }
    /**
     * Git 변경 감지 설정
     */
    ...
    async createCommit() {
        ...
        try {
            ...
            try {
                ...
                vscode.window.showInformationMessage('아틀라시안 API 설정이 올바르지 않거나 누락되었습니다. 설정에서 확인해보세요.', openSettings).then(selection => {
...
설명만 제공하고 다른 설명은 하지 마세요. 가능하면 72자 미만으로 작성하세요.`;
                ...
                const result = await vscode.window.showInformationMessage(`${commitCount}개의 임시 커밋을 통합하시겠습니까?`, { modal: true }, '통합하기', '취소');
                if (result !== '통합하기') {
                    return;
                }
                ...
                // 성공 메시지
                vscode.window.showInformationMessage(`${commitCount}개의 임시 커밋이 성공적으로 통합되었습니다.`);
                // 상태 업데이트
                this.updateStatusBar();
            });
        }
...
exports.AutoCommitService = AutoCommitService;
//# sourceMappingURL=autoCommitService.js.map