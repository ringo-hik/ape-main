...
exports.ChatViewService = void 0;
/**
 * 채팅 뷰 서비스 클래스
 */
class ChatViewService {
     ...
     * @returns 스크립트 문자열
     */
    static getChatViewScript(initialMessages, isStreaming) {
        console.log('ChatViewService.getChatViewScript called:', `initialMessages: ${initialMessages.length} items, isStreaming: ${isStreaming}`);
        return `
    // Initial data
    ...
    function updateMessages(messages, isStreaming) {
      ...
      messages.forEach(message => {
        ...
        if (!messageElement) {
          ...
          copyButton.innerHTML = '<i class="codicon codicon-copy"></i><span class="tooltip">전체 답변 복사</span>';
    ...
    function copyMessageToClipboard(messageId) {
      ...
      try {
        ...
        navigator.clipboard.writeText(textContent).then(() => {
          ...
          showNotification('메시지 복사에 실패했습니다.', 'error');
        });
      } catch (err) {
        console.error('메시지 복사 중 오류 발생:', err);
        showNotification('메시지 복사 중 오류가 발생했습니다.', 'error');
      }
    }
  ...
  `;
     ...
     * @returns 변환된 내용
     */
    static replaceCodeBlocks(content, options) {
     ...
     * @returns 포맷팅된 HTML
     */
    static formatCodeBlock(codeContent, language, options) {
...
//# sourceMappingURL=chatViewService.js.map