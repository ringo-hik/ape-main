...
# APE Extension Webpack 빌드 스크립트
# Webpack 기반 빌드 및 번들링을 수행합니다.
...
echo -e "${GREEN}===== APE Extension v0.4.27 Webpack 빌드 스크립트 =====${NC}"
...
if [ "$DIR_NAME" != "ape-extension" ]; then
  echo -e "${YELLOW}경고: 현재 디렉토리가 'ape-extension'이 아닙니다. 현재 경로: ${CURRENT_DIR}${NC}"
fi
...
if [ "$1" == "prod" ] || [ "$1" == "production" ]; then
  ...
  echo -e "${YELLOW}프로덕션 모드로 빌드합니다.${NC}"
else
  echo -e "${YELLOW}개발 모드로 빌드합니다. 프로덕션 모드로 빌드하려면 'prod' 인자를 추가하세요.${NC}"
fi
...
npm run lint || { 
  echo -e "${YELLOW}린트 오류가 있습니다. 코드를 수정한 후 다시 시도하세요.${NC}"
...
# 웹팩 빌드 실행
echo -e "${GREEN}Webpack 빌드 실행 중 (${BUILD_MODE} 모드)...${NC}"
if [ "$BUILD_MODE" == "production" ]; then
  npm run webpack:prod || { 
    echo -e "${RED}빌드 실패! 오류를 수정한 후 다시 시도하세요.${NC}" >&2
    exit 1
  }
else
  npm run webpack || { 
