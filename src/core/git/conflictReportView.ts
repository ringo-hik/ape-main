...
import { ConflictSolver, ConflictStrategy } from './conflictSolver';
import { LLMService } from '../llm/llmService';

/**
 * 충돌 해결 제안 정보
 */
interface ConflictResolutionProposal {
  filePath: string;
  relativePath: string;
 ...
 * 충돌 보고서 웹뷰 제공자
 */
export class ConflictReportViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'apeConflictReport';
  private _view?: vscode.WebviewView;
  private _proposals: ConflictResolutionProposal[] = [];
  private _extensionUri: vscode.Uri;
  private _workspaceRoot: string | undefined;
  private _pendingConflictCount: number = 0;
  ...
  constructor(
    ...
    private readonly conflictSolver: ConflictSolver
  ) {
   ...
   * 충돌이 있는지 확인 및 보고서 생성
   */
  public async checkForConflictsAndReport(): Promise<number> {
    // 충돌 파일 목록 가져오기
   ...
   * 충돌 제안 생성
   */
  private async _generateProposal(filePath: string, relativePath: string): Promise<void> {
    try {
      // 파일 확장자 확인
      ...
      this._proposals.push({
        ...
        strategy: ConflictStrategy.LLM, // 기본 전략
        approved: false
      });
   ...
   * 제안 승인
   */
  private async _approveProposal(index: number): Promise<void> {
   ...
   * 제안 거부
   */
  private async _rejectProposal(index: number): Promise<void> {
    if (index >= 0 && index < this._proposals.length) {
   ...
   * 모든 제안 승인
   */
  private async _approveAllProposals(): Promise<void> {
   ...
   * 승인된 제안 적용
   */
  private async _applyApprovedProposals(): Promise<void> {
      ...
      for (let i = 0; i < approvedProposals.length; i++) {
        ...
        try {
          ...
          await vscode.workspace.fs.writeFile(
            vscode.Uri.file(proposal.filePath),
            Buffer.from(proposal.proposed, 'utf-8')
          );
   ...
   * 제안 전략 변경
   */
  private async _changeStrategy(index: number, strategy: ConflictStrategy): Promise<void> {
    if (index >= 0 && index < this._proposals.length) {
   ...
   * 제안 재생성
   */
  private async _regenerateProposal(index: number): Promise<void> {
    if (index >= 0 && index < this._proposals.length) {
      const proposal = this._proposals[index];
      ...
      for (const conflict of conflictInfo.conflicts) {
        // 항상 LLM 전략 사용
        const mergeResult = await this.conflictSolver.applyStrategy(conflict, ConflictStrategy.LLM, fileExt);
      ...
      // 제안 업데이트
      proposal.proposed = resolvedContent;
      proposal.strategy = ConflictStrategy.LLM;
      proposal.approved = false;
   ...
   * 제안 직접 수정
   */
  private async _editProposal(index: number, content: string): Promise<void> {
    if (index >= 0 && index < this._proposals.length) {
      this._proposals[index].proposed = content;
      this._proposals[index].strategy = ConflictStrategy.LLM; // 사용자 편집은 LLM 전략으로 표시
      this._proposals[index].approved = false;
   ...
   * 웹뷰 업데이트
   */
  private _updateView(message: any): void {
    if (this._view) {
  ...
  private _getHtmlForWebview(webview: vscode.Webview): string {
    ...
    const styleUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'media', 'chat-ape.css')
    );

    const codiconsUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'media', 'codicon', 'codicon.css')
    );
